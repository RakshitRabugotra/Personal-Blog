...
export default function Footer() {
  return (
    <footer className="w-full mt-8" {...thisSection}>
      <nav className="flex flex-row flex-wrap max-w-[50vw] justify-around mx-auto">
        {Object.entries(SECTIONS).map(([section, { id, showInNav }], index) => {
            if(!showInNav) return <a className="hidden" key={index}></a>
          return (
            <a href={"#" + id} key={index} className="capitalize">
              {section}
            </a>
          );
        })}
      </nav>
      <Text className="text-center">{STRINGS.FOOTER.text}</Text>
    </footer>
  );
}
